...
y_save<-y[keep,]
write.table(cpm(y),"Gene.norm.txt",sep="\t",quote=FALSE)
...
for (i in 1:4){
	...
	de<-decideTestsDGE(lrt,adjust.method="BH",p.value=1e-5)
	...
	fupgene = gsub(" ","_",paste("Results/",mymain,".up.txt",sep=""))
	fdngene = gsub(" ","_",paste("Results/",mymain,".dn.txt",sep=""))
